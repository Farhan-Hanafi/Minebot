...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "🚚 Find Hauler Cycle Time\n"
        "/cycletimehauler\n\n"

        "Type any command and MineBot will provide an input form."
    )
...
async def cycletimehauler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        empty_speed = float(context.args[1])
        load_speed = float(context.args[2])
        est = float(context.args[3])
        lst = float(context.args[4])
        disp_time = float(context.args[5])
        loading_time = float(context.args[6])

        cycle_time = (
            (distance / empty_speed)
            + (distance / load_speed)
            + est
            + lst
            + disp_time
        ...
        await update.message.reply_text(
            "🚚 Actual Hauler Cycle Time\n\n"
            f"Distance = {distance:.2f} km\n"
            f"Empty Speed = {empty_speed:.2f} km/min\n"
            f"Load Speed = {load_speed:.2f} km/min\n"
            f"EST = {est:.2f} min\n"
            f"LST = {lst:.2f} min\n"
            f"Disposal Time = {disp_time:.2f} min\n"
            f"Loading Time = {loading_time:.2f} min\n\n"
            f"🔄 Cycle Time = {cycle_time:.2f} minutes"
        )

    except:
        await update.message.reply_text(
            "Usage:\n"
            "/cycletimehauler <Distance> <EmptySpeed> <LoadSpeed> <EST> <LST> <DispTime> <LoadingTime>\n\n"
            "Example:\n"
            "/cycletimehauler 2.5 0.8 0.6 0.5 0.5 1 2.5"
        )
